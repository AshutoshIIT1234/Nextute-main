...
export const uploadStudentProfileImage = async (req, res) => {
  try {
    ...
    if (!req.file || !req.file.path) {
      return handleError(res, 400, "No image file uploaded", "NO_IMAGE_UPLOADED");
    }
    // Save image URL to student profile
    ...
    return handleError(res, 500, "Server error during profile image upload", "PROFILE_IMAGE_UPLOAD_ERROR");
  }
};
...
import { findStudentByPhone, createStudent, verifyStudent, getAllStudents, findStudentById, findStudentByEmail, deleteStudentByEmail } from "../models/studentModel.js";
...
export const fetchStudentById = async (req, res) => {
  ...
  try {
    if (!isUuid(id)) {
      return res.status(400).json({ message: 'Invalid student id format. Must be a UUID.' });
    }
...
export const editStudentById = async (req, res) => {
  ...
  console.log('Student update payload:', updateData);
